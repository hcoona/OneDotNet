...
namespace RateLimiter
{
    internal sealed class SmoothWarmingUpRateLimiter : SmoothRateLimiter
    {
        ...
        protected override void DoSetRate(double permitsPerSecond, TimeSpan stableInterval)
        {
            ...
            this.maxPermits =
                this.thresholdPermits
                    + (2.0 * this.warmupPeriod.Ticks / (stableInterval + coldInterval).Ticks);
            this.slope =
                (coldInterval - stableInterval).Divide(this.maxPermits - this.thresholdPermits);
            ...
            double permitsToTake)
        {
            ...
            {
                var permitsAboveThresholdToTake =
                    Math.Min(availablePermitsAboveThreshold, permitsToTake);
                var length =
                    this.PermitsToTime(availablePermitsAboveThreshold)
                        + this.PermitsToTime(
